...
\begin{abstract}
\noindent
SAT solvers underpin every modern formal verification flow, but
Boolean Constraint Propagation (BCP) is well known to consume the
overwhelming majority of CDCL runtime.\footnote{S. A. Seshia, EECS~219C
lecture notes, ``Boolean Satisfiability Solving.''
\url{https://people.eecs.berkeley.edu/~sseshia/219c/lectures/SATSolving.pdf}}
We design a hardware--software coprocessor that pushes BCP into a
CAM/SRAM RTL engine while keeping CDCL search logic (VSIDS, 1--UIP,
non--chronological backtracking) in a minimal C++ wrapper, simulated
through Verilator. On SATLIB Uniform Random 3--SAT phase--transition
benchmarks, the coprocessor delivers \textbf{37--341$\times$} faster
BCP than Kissat~4.0.4, outperforms Kissat, MiniSat2, and Glucose
end--to--end up to $\sim$175 variables, and reveals that beyond that
point the bottleneck shifts to the CDCL heuristic --- confirmed by a
counter--factual that re--runs the search with software BCP rates.
\end{abstract}
...
\subsection{Coprocessor algorithm}
The CDCL outer loop is essentially MiniSat--style 1--UIP, but every
\texttt{propagate()} call traps into the RTL (Algorithm~\ref{alg:co}).

\begin{algorithm}[h]
\small
\caption{HW/SW Coprocessed CDCL}\label{alg:co}
\begin{algorithmic}[1]
\STATE Load all original clauses via \texttt{LOAD}
\WHILE{true}
  \STATE $r \leftarrow$ HW-BCP() \quad \emph{(single--cycle parallel scan)}
  \IF{$r =$ \textsc{conflict}}
    \IF{decision level $=0$} \STATE \textbf{return} UNSAT \ENDIF
    \STATE $(c_{\text{learnt}}, lvl) \leftarrow$ Analyze1UIP$()$
    \STATE Backtrack$(lvl)$ via \texttt{UNDO}
    \STATE LoadLearnt$(c_{\text{learnt}})$ \quad \emph{(or overflow $\to$ SW pool)}
  \ELSIF{$r =$ \textsc{unit}$(\ell)$}
    \STATE Enqueue $\ell$ on trail; \texttt{LOAD} updates SRAM
  \ELSE
    \IF{trail covers all vars} \STATE \textbf{return} SAT \ENDIF
    \STATE $v \leftarrow$ VSIDS-Pick$()$; assign and \texttt{LOAD}
  \ENDIF
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
Figure~\ref{fig:walk} shows a single propagation step for
$C_0 = x_1 \vee x_2 \vee x_3$. The trail head is $x_2 = 0$. The CAM
returns rows referencing $x_1, x_2, x_3$ for $C_0$; the SRAM resolves
$x_1=0, x_2=0, x_3=U$; the per--clause evaluator detects exactly one
unassigned literal with the rest false, so it asserts \textsc{unit}
and emits $x_3 \gets 1$ in the same cycle.

\begin{figure}[h]
\centering
\begin{tikzpicture}[every node/.style={font=\scriptsize\sffamily}]
  \matrix[matrix of nodes, nodes={draw,minimum width=0.9cm,minimum height=0.55cm,anchor=center},
          row sep=-\pgflinewidth,column sep=-\pgflinewidth] (cam) at (0,0) {
   \textbf{var} & \textbf{cls} & \textbf{idx} \\
   $x_1$ & $C_0$ & 0 \\
   $x_2$ & $C_0$ & 1 \\
   $x_1$ & $C_1$ & 0 \\
   $x_3$ & $C_1$ & 1 \\
  ...
  \matrix[matrix of nodes, nodes={draw,minimum width=0.9cm,minimum height=0.55cm,anchor=center},
          row sep=-\pgflinewidth,column sep=-\pgflinewidth] (sram) at (3.5,0) {
   \textbf{var} & \textbf{val} \\
   $x_1$ & 0 \\
   $x_2$ & 0 \\
   $x_3$ & U \\
  ...
  \node[draw,rounded corners,fill=yellow!15,align=left] at (7.0,0)
   {$C_0 = x_1 \vee x_2 \vee x_3$\\
    $x_1=0\;\textcolor{red}{\times}$\\
    $x_2=0\;\textcolor{red}{\times}$\\
    $x_3=U\;\Rightarrow$\\
    \textbf{UNIT}: $x_3 \gets 1$};
\end{tikzpicture}
\caption{Single--cycle BCP: CAM returns matching rows, SRAM gives
literal values, evaluator detects \textsc{unit}.}
\label{fig:walk}
\end{figure}
...
\subsection{Small but provably hard instances}\label{sec:hard}
Phase--transition random--3--SAT is \emph{statistically} hard;
worst--case CDCL is set by adversarial classes admitting only
super--polynomial resolution proofs. We built a small suite at
\texttt{tests/satlib/hard\_instances/} (clauses wider than four
literals are Tseitin--split to fit the 4--lit RTL):
pigeonhole PHP$_n$ ($n{=}3{\ldots}8$, the canonical Haken
exponential--resolution lower bound); Tseitin parity on odd cycles
($C_5,C_7,C_9$); and $k$--colourability of the Grötzsch graph
($\chi{=}4$, so $k{=}3$ is hard UNSAT and $k{=}4$ is SAT).
...
gap between HW--BCP and the SW solvers \emph{widens} as $n$ grows:
on PHP$_8$, HW is $5.9\times$ faster than Kissat, $28\times$ faster
than MiniSat2, and $142\times$ faster than Glucose --- on a class
where the search space itself is exponential, the constant--time
HW--BCP cycle compounds across every conflict in the proof.}
\label{fig:hard}
\end{figure}

\paragraph{Discussion.} On PHP, every solver must enumerate the same
exponentially many conflicts; the win is decided by
\emph{cost--per--propagation}. SW two--literal watching degrades on
dense binary clause bases (PHP is mostly width--$2$) while HW
parallel re--evaluation pays no such penalty --- the same effect
behind the $30$--$90\times$ wins on Tseitin and Grötzsch. PHP$_3$/PHP$_5$
solve below the wall--clock floor for some SW solvers, so we make
no claim there. PHP$_9$ and beyond would push us into the overflow
pool and erode the constant--cycle BCP, which the $1$M--slot
production target eliminates.
...
the asserting level via \texttt{UNDO} cycles --- all directly from
class.
\item \textbf{Phase transitions and resolution proofs.} SATLIB
UF/UUF at $c/v\approx 4.27$ sits on the empirical 3--SAT hardness
peak. The hard--instance suite (Section~\ref{sec:hard}) leverages
the resolution--proof framing: any CDCL refutation of pigeonhole
encodes a resolution proof, and pigeonhole has only
super--polynomial ones --- so conflict count is invariant across
solvers and HW wins on cost--per--propagation.
...
generator and CNFs): \texttt{[INSERT LINK]}.
